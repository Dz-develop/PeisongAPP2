...
import com.example.peisongapp.base.PeiSongApp;
import com.example.peisongapp.base.PeiSongConstants;
...
public class Utils {
	 ...
	 *            显示内容
	 */
	public static void showCopyDialog(final Context context,
				...
				.setItems(new String[] { "复制" },
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								PeiSongApp.getClipboardManager().setText(
										content);
								dialog.dismiss();
							}
						}).show();
	...
	public static void toast(Context c, String msg, boolean show, int duration) {
		if (!show)
			return;
		PeiSongApp.toastMgr.builder.display(msg, duration);
	}
	...
	public static void toast(Context c, int resid, boolean show, int duration) {
		if (!show)
			return;
		PeiSongApp.toastMgr.builder.display(resid, duration);
	}
	...
	public static File file_put_contents(String urlStr, InputStream is) {
		...
		// urlStr.hashCode();
		String path = PeiSongConstants.PIC_CACHE_DIR_PATH + File.separator
				+ urlStr.hashCode();
		File file = new File(path);
		if (!file.getParentFile().exists()) {
			file.getParentFile().mkdirs();
